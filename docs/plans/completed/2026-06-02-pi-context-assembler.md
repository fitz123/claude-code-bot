...
- [x] Add a short section to `README.md` (or `bot/src/pi-extensions/README.md` if that's the home for Pi-spawn docs) describing the context assembler: what it delivers, the layer mapping (`--system-prompt` persona / `--append-system-prompt` bundle / `--no-context-files`), fail-safe, and that memory auto-recall is a tracked fast-follow. (Added a "Context assembler (Claude→Pi parity at spawn)" section to `bot/src/pi-extensions/README.md` — the Pi-spawn docs home; root `README.md` is the upstream immutable-core file. Covers the layer-mapping table, deterministic bundle order 1-5, fail-safe, the mtime/size cache, atomic `.tmp/` artifacts, and the memory_search fast-follow note.)
- [x] Ensure code comments in `pi-context-assembler.ts` document the deterministic bundle order + the 1-level import policy. (Already present and verified: the module header docblock enumerates the deterministic bundle order 1-5 and the "1 level only — nested @-line NOT recursed, only warned" policy; `expandImports` re-documents the 1-level/no-recursion + missing-import-skip rules; `assembleBundle`/`buildBundle` reference "order 1-5 above".)
