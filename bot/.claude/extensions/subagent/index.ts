...
// thin jiti-only orchestrator over it.
import {
...
async function runSingleAgent(
	...
	// Map a streaming/terminal child run into this extension's SingleResult shape.
	const toSingle = (r: SubagentRunResult): SingleResult => ({
		agent: agentName,
			...
			onMessage: onUpdate
				? (r) =>
						onUpdate({
							...
							details: makeDetails([toSingle(r)]),
						})
				: undefined,
			warn: (e: SubagentChildErrorWarn) => {
				// RPC mode has no UI; surface the child error on stderr so it lands
				// in the parent Pi session's logs (parity with the web-tools wrapper).
				// eslint-disable-next-line no-console -- structured warn-log for the Pi session
				console.warn(formatSubagentChildErrorWarn(e));
			},
		});
