...
/**
 * Build a PlatformContext from the Bot API instead of an incoming message
 * Context. Used by deliver.sh echo routing, where there is no inbound update
 * but a context-only agent prompt may still need to relay an accidental reply
 * or outbox file to the target chat.
 */
export function createTelegramApiAdapter(
  ...
  const threadOpts = threadIdOverride != null ? { message_thread_id: threadIdOverride } : {};
  const isDm = binding?.kind === "dm";
  ...
  return {
    ...
    async sendMessage(text: string): Promise<string> {
      ...
      try {
        ...
        if (threadIdOverride != null) setThread(chatId, sent.message_id, threadIdOverride);
    ...
    async sendTyping(): Promise<void> {
      await api.sendChatAction(
        chatId,
        "typing",
        threadIdOverride != null ? { message_thread_id: threadIdOverride } : undefined,
      );
    },
