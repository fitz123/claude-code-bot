...
Hooks live in agent workspaces (e.g., `/Users/user/.minime/workspace/.claude/settings.json`), NOT in bot repo. Current hooks: `protect-files.sh`, `guardian.sh` (PreToolUse, matcher `Edit|Write`), `auto-stage.sh` (PostToolUse).
...
- Hooks live in AGENT workspaces (`/Users/user/.minime/workspace/.claude/settings.json`), not bot repo

**Important:** This affects both `main` agent workspace (`/Users/user/.minime/workspace/`) and `coder` workspace (`/Users/user/.minime/workspace-coder/`). Both need the hook installed.
