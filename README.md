...
|---|---|---|
| **A1 guard** | `bot/.claude/extensions/guardian-protect-files.ts` | A `tool_call` handler that blocks edit/write and bash redirects (`>`, `>>`, `tee`, `mv`, `cp`) into upstream-owned paths (`bot/`, `.claude/rules/platform/`, `.github/workflows/`, `.githooks/`), `..` traversal escapes, and (for `write`) creation of new root-level entries outside the `orphan-allowlist.txt` workspace structure — guardian.sh parity. Path matching canonicalizes `.`/`..`/`//` and is case-insensitive (APFS), including the workspace-root prefix of absolute paths. Fails **closed** — an unknown workspace root blocks. |
